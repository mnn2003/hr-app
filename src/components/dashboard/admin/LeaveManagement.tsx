import { useState, useEffect, useMemo } from 'react'; import { collection, getDocs, doc, updateDoc, getDoc, setDoc, writeBatch, } from 'firebase/firestore'; import { db } from '@/lib/firebase'; import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card'; import { Button } from '@/components/ui/button'; import { Badge } from '@/components/ui/badge'; import { Input } from '@/components/ui/input'; import { Label } from '@/components/ui/label'; import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogTrigger, } from '@/components/ui/dialog'; import { toast } from 'react-hot-toast'; import { User, Edit, Plus, AlertCircle, Search } from 'lucide-react'; import { LeaveBalance, LeaveType } from '@/types/leave'; import { Alert, AlertDescription } from '@/components/ui/alert';

// ---------------------------------------------------------------------------------- // Constants // ---------------------------------------------------------------------------------- const DEFAULT_LEAVE_BALANCE = { PL: 30, CL: 2, SL: 7, WFH: 15, MATERNITY: 182, PATERNITY: 14, ADOPTION: 84, SABBATICAL: 0, BEREAVEMENT: 10, PARENTAL: 10, COMP_OFF: 0, };

const LeaveManagement = () => { const [employees, setEmployees] = useState([]); const [loading, setLoading] = useState(true); const [selectedEmployee, setSelectedEmployee] = useState(null); const [editBalance, setEditBalance] = useState(null); const [isDialogOpen, setIsDialogOpen] = useState(false); const [lastAllocation, setLastAllocation] = useState(null); const [canAllocate, setCanAllocate] = useState(true); const [queryText, setQueryText] = useState(''); const [expandedEmployee, setExpandedEmployee] = useState(null);

useEffect(() => { let mounted = true; (async () => { await fetchEmployeesWithBalances(); await checkLastAllocation(); if (mounted) setLoading(false); })(); return () => { mounted = false; }; }, []);

// ---------------------------------------------------------------------------------- // Read/Write helpers // ---------------------------------------------------------------------------------- const checkLastAllocation = async () => { try { const allocationDoc = await getDoc(doc(db, 'system_settings', 'leave_allocation')); if (allocationDoc.exists()) { const lastDate = allocationDoc.data().lastAllocation; setLastAllocation(lastDate);
...
const fetchEmployeesWithBalances = async () => { try { setLoading(true); const employeesSnapshot = await getDocs(collection(db, 'employees'));

// fetch balances in parallel but limited batching would be ideal for very large data sets
...
// ---------------------------------------------------------------------------------- // Edit & Update // ---------------------------------------------------------------------------------- const handleEditBalance = (employee) => { setSelectedEmployee(employee); // deep clone to avoid mutating the list directly setEditBalance(JSON.parse(JSON.stringify(employee.balance))); setIsDialogOpen(true); };

const handleUpdateBalance = async () => { if (!selectedEmployee || !editBalance) return;
...
const handleBalanceChange = (leaveType, value) => { if (!editBalance) return; const numeric = parseFloat(value); setEditBalance({ ...editBalance, [leaveType]: Number.isNaN(numeric) ? 0 : numeric, }); };

// ---------------------------------------------------------------------------------- // Monthly allocation (batched) // ---------------------------------------------------------------------------------- const handleAllocateLeaves = async () => { if (!canAllocate) { toast.error('Monthly leaves have already been allocated this month'); return; }

const confirmed = window.confirm(
  'Allocate monthly leaves to all employees? This can only be done once per month.'
);
if (!confirmed) return;
...
// ---------------------------------------------------------------------------------- // Filtering // ---------------------------------------------------------------------------------- const filteredEmployees = useMemo(() => { const q = queryText.trim().toLowerCase(); if (!q) return employees; return employees.filter((e) => ${e.name} ${e.employeeCode} ${e.department}.toLowerCase().includes(q) ); }, [employees, queryText]);

// ---------------------------------------------------------------------------------- // Small UI helpers // ---------------------------------------------------------------------------------- const toggleExpand = (id) => setExpandedEmployee((prev) => (prev === id ? null : id));

return ( <div className="space-y-4 max-w-5xl mx-auto p-4"> <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4"> <div> <h2 className="text-2xl font-semibold">Leave Balance Management</h2> <p className="text-sm text-muted-foreground">Quickly view and update employee leave balances.</p> </div>
  ...
  {!canAllocate && lastAllocation && (
    <Alert>
      ...
      <AlertDescription className="ml-2">
        Monthly leaves were already allocated on {new Date(lastAllocation).toLocaleDateString()}. You can allocate
        again next month.
      </AlertDescription>
    </Alert>
  )}
